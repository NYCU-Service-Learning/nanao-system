...
import { getAuthConfig, type AuthConfig } from "../../api/authConfigAPI";
...
const Login: React.FC = () => {
  ...
  useEffect(() => {
    const msgGoogle = handleOAuthLogin(
      "googleLogin",
      "Google 登入失敗，請重試",
    );
  ...
  const handleLineLogin = () => {
    if (!authConfig?.auth.lineLogin) {
      setErrMsg("Line 登入服務未設置");
      return;
    }
      ...
      {cookies.user ? (
        ...
        <div className="center">
          <div className="login">
            ...
            {authConfig && (
              <button
                className={`gsi-material-button ${!authConfig.auth.google ? "disabled" : ""}`}
                onClick={handleGoogleLogin}
                disabled={!authConfig.auth.google}
                type="button"
              >
            ...
            {authConfig && (
              <button
                className={`gsi-material-button ${!authConfig.auth.lineLogin ? "disabled" : ""}`}
                onClick={handleLineLogin}
                disabled={!authConfig.auth.lineLogin}
                type="button"
              >
                <div className="gsi-material-button-state"></div>
                <div className="gsi-material-button-content-wrapper">
