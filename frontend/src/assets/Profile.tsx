  ...
  useEffect(() => {
    if(userData){
      if(userData.headshot !== "0"){
        setAvatarUrl(`http://localhost:8001/avatar_styled/styled-ca${userData.headshot}-${userId}.jpg`);
      }
      if(userData.headshot === "4"){
        setAvatarUrl(`http://localhost:8001/avatar_original/original-${userId}.jpg`);
      }
    }
  }, [userData, userId]);
