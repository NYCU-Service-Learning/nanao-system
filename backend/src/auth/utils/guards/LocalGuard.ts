...
export class AdminOrSameUserIdGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext) {
    ...
    const userId_params_name = this.reflector.get<string>(
      'userId',
      context.getHandler(),
    );
...
export class FormDeleteGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext) {
    ...
    const formType = this.reflector.get<string>(
      'formType',
      context.getHandler(),
    );
    const formIdName = this.reflector.get<string>(
      'formIdName',
      context.getHandler(),
    );
    ...
    if (isNaN(formId)) {
      throw new HttpException(
        'form_id parameter has invalid format',
    ...
    if (form.user_id == req.user['id']) return true;
    return false;
  }
}
